...
def compute_state(now: datetime, repo: str) -> dict[str, Any]:
    ...
    if active:
        ...
        # A verified jobs.json that has not reached the user-facing unified dataset
        # is already a publication incident. Drain that backlog before starting
        # another long Fast refresh; unified-search is fail-closed and preserves
        # last-known-good data when validation fails.
        if unified_publication_stale(jobs_time, unified_time) and not fast_status_running:
            ...
            if blocked:
                ...
                action = "unified"
                reason = (
                    "publish verified jobs backlog before another Fast refresh: "
